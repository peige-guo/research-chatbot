...
if os.getenv("DEEPSEEK_API_KEY") is None:
    raise ValueError(
        "DEEPSEEK_API_KEY is not set. Please create a .env file in the backend directory "
        "with your DeepSeek API key. Get your key from https://platform.deepseek.com/"
    )

if os.getenv("TAVILY_API_KEY") is None:
    raise ValueError(
        "TAVILY_API_KEY is not set. Please create a .env file in the backend directory "
        "with your Tavily API key. Get your key from https://tavily.com/"
    )
...
async def web_research(state: WebSearchState, config: RunnableConfig) -> OverallState:
    ...
    analysis_prompt = f"{formatted_prompt}\n\nSearch Results:\n{search_content}\n\nPlease analyze these search results and provide a comprehensive summary with citations. Please answer in English."
